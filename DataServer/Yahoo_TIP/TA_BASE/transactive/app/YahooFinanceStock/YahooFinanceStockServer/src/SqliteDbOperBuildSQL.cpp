...
QString CSqliteDbOperBuildSQL::buildSQL_Select_TABLE_USER_ACCOUNT( const QString& strUserID )
{
	QString  strSQL;
	std::stringstream byteSQL;
	...
	byteSQL<<"SELECT"
		...
		<<" "<<"WHERE"
		<<" "<<str_TABLE_USER_ACCOUNT_COLUMN_USEID<<"="<<"\""<<strUserID.toStdString()<<"\""
		<<" "<<"ORDER BY"
		<<" "<<str_TABLE_USER_ACCOUNT_COLUMN_UPDATE_TIME
		<<" "<<"ASC";//ASC min->max   DESC max->min
	strSQL = byteSQL.str().c_str();
...
QString CSqliteDbOperBuildSQL::buildSQL_Select_TABLE_USER_HOLD_ACCOUNT(const QString& strUserID)
{
	...
	byteSQL<<"SELECT"
		...
		<<" "<<"WHERE"
		<<" "<<str_TABLE_USER_HOLD_ACCOUNT_COLUMN_USERID<<"="<<"\""<<strUserID.toStdString()<<"\""
		<<" "<<"ORDER BY"
		<<" "<<str_TABLE_USER_HOLD_ACCOUNT_COLUMN_SYMBOLUSE
		<<" "<<"ASC";
	strSQL = byteSQL.str().c_str();
...
QString CSqliteDbOperBuildSQL::buildSQL_Select_TABLE_USER_HOLD_ACCOUNT(const QString& strUserID, const QString& strSymbolUse)
{
	...
	byteSQL<<"SELECT"
		...
		<<" "<<str_TABLE_USER_HOLD_ACCOUNT_COLUMN_USERID<<"="<<"\""<<strUserID.toStdString()<<"\""
		<<" "<<"AND"
		<<" "<<str_TABLE_USER_HOLD_ACCOUNT_COLUMN_SYMBOLUSE<<"="<<"\""<<strSymbolUse.toStdString()<<"\"";
	strSQL = byteSQL.str().c_str();
	return strSQL;	
}
