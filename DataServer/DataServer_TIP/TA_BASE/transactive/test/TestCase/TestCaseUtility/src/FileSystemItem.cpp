...
CFileSystemItem::CFileSystemItem( const std::string& strFilePath )
{
	...
	//m_nLastWriteTime = boost::filesystem::last_write_time(m_fFileFullPath);//std::ctime(&t)
	m_nLastWriteTime = boost::lexical_cast<int>(m_strFileName);
}
...
std::time_t CFileSystemItem::getLastWriteTime()
{
	return m_nLastWriteTime;
}


std::string CFileSystemItem::getFileName()
{
