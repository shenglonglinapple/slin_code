...
{
	BOOST_LOG_FUNCTION();
	m_pServerWorkerForTest->handleDeliverFailure(pGetMessage);
...
int CServerManager::_InitServer()
{
	BOOST_LOG_FUNCTION();
	int nFunRes = 0;
	std::string strServerAddress = CConfigInfo::getInstance().getServerAddress();

	if (strServerAddress.empty())
	{
		LOG_ERROR<<"error! strServerAddress="<<strServerAddress;
		nFunRes = -1;
		return nFunRes;
	}

	{
		...
		LOG_DEBUG<< "begin listen strServerAddress="<<strServerAddress;
		m_brokerServer.listen(strServerAddress);
	}
