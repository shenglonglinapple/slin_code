...
{	
	...
	m_strServerAddress = CConfigInfo::getInstance().getServerAddress();
...
void CServerWorkerForTest::handleDeliverFailure(Message::Ptr pGetMessage)
{
	...
	strLogInfo = "CServerWorkerForTest::handleDeliverFailure:";

	LOG_ERROR<<strLogInfo
		<<" "<<"pGetMessage=0x"<<pGetMessage
		<<" "<<"pGetMessage->getClientID="<<pGetMessage->getClientID();
...
int CServerWorkerForTest::_Process_InitServer()
{
	...
	if (m_strServerAddress.empty())
	{
		LOG_ERROR<<"error! m_strServerAddress="<<m_strServerAddress;
		nFunRes = -1;
		return nFunRes;
	}
...
{
	LOG_INFO<<"ServerAddress="<<m_strServerAddress
...
void CServerWorkerForTest::_AddClient(const SessionInfo &stSessionInfo)
{
	...
	//log
	std::string strLogInfo;
	strLogInfo = m_strServerAddress + " accept new client: ";
	m_pHelpClass->log_SeeionINfo(stSessionInfo, strLogInfo);
	LOG_INFO<<"m_nClientsCount="<<m_nClientsCount;
}
