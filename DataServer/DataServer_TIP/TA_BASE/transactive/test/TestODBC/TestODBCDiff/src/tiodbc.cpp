...
namespace tiodbc
{
	...
	{
		bool bFunRes = true;
		SQLRETURN nAPIFunRes = SQL_SUCCESS;
		...
		//set AUTOCOMMIT OFF
		nAPIFunRes = SQLSetConnectOption(conn_h, SQL_AUTOCOMMIT, SQL_AUTOCOMMIT_OFF);
		if (!TIODBC_SUCCESS_CODE(nAPIFunRes))
		{	
			bFunRes = false;
		}
	...
	bool connection::setAutoCommitOn()
	{
		bool bFunRes = true;
		SQLRETURN nAPIFunRes = SQL_SUCCESS;
		...
		//set SQL_AUTOCOMMIT_ON
		nAPIFunRes = SQLSetConnectOption(conn_h, SQL_AUTOCOMMIT, SQL_AUTOCOMMIT_ON);
		if (!TIODBC_SUCCESS_CODE(nAPIFunRes))
		{	
			bFunRes = false;
		}
	...
	bool connection::rollback()
	{
		...
		nAPIFunRes = SQLEndTran(SQL_HANDLE_DBC, conn_h, SQL_ROLLBACK);
		if (!TIODBC_SUCCESS_CODE(nAPIFunRes))
		{	
			bFunRes = false;
		}
	...
	bool connection::commit()
	{
		...
		nAPIFunRes = SQLEndTran(SQL_HANDLE_DBC, conn_h, SQL_COMMIT);
		if (!TIODBC_SUCCESS_CODE(nAPIFunRes))
		{	
			bFunRes = false;
		}
	...
	{
		bool bFunRes = true;
		RETCODE nAPIFunRes = SQL_SUCCESS;
		...
		_conn.setAutoCommitOff();

		// Execute directly statement
		nAPIFunRes = SQLExecDirect(stmt_h, (SQLTCHAR *)_query.c_str(), SQL_NTS);
		if (!TIODBC_SUCCESS_CODE(nAPIFunRes))
		{
			_conn.rollback();
			bFunRes = false;
