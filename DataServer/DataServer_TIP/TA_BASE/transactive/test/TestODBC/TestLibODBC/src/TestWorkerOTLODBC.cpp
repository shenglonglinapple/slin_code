...
CTestWorkerOTLODBC::CTestWorkerOTLODBC( void )
{
	m_nDBType = DB_ORACLE;
	m_pConnection = NULL;
...
void CTestWorkerOTLODBC::SetDBType( enDBType nDBType )
{
	m_nDBType = nDBType;
	m_pBuildSQLHelper->setDBType(m_nDBType);
...
//////////////////////////////////////////////////////////////////////////
template<class T>
std::string CTestWorkerOTLODBC::t_to_string(const T& t)
{
	std::string strValue;
...
int CTestWorkerOTLODBC::_GetColumnCount(otl_stream* pQuery)
{
...
std::string CTestWorkerOTLODBC::_GetColumnName(otl_stream* pQuery, int nColumnIndex)
{
	std::string strValue;
	otl_column_desc* p_otl_column_desc = NULL;
	int nColumnCount = 0;//nColumnCount;
...
std::string CTestWorkerOTLODBC::_GetColumnValue(otl_stream* pQuery, int nColumnIndex)
{
	std::string strValue;
	otl_column_desc* p_otl_column_desc = NULL;
	int nColumnCount = 0;//nColumnCount;
...
void CTestWorkerOTLODBC::fillDataBuffer(otl_connect *pconnection, otl_stream* pQuery, int numColumns, long lMaxRows )
{
	int nColumnCount = 0;//nColumnCount;
	int nRowIndex = 0;
...
void CTestWorkerOTLODBC::fetchInitial(otl_connect *pconnection, otl_stream* pQuery, int numColumns, long lMaxRows )
{
	try
	{
		{
...
void CTestWorkerOTLODBC::exec(otl_connect *pconnection, const std::string& strOracleSQL, bool isQuery)
{
	int prefetch = 100;
...
int CTestWorkerOTLODBC::do_do_init()
{
	...
	try
	{
		...
		//db.rlogon("UID=IT271350_5;PWD=IT271350_5;DSN=DEV_C225"); // connect to ODBC
		db.rlogon("IT271350_5/IT271350_5@DEV_C225", 0); // connect to ODBC
		db.auto_commit_off();


// 		otl_cursor::direct_exec(db,	"drop table test_tab",otl_exception::disabled); // drop table
// 		otl_cursor::direct_exec(db,"create table test_tab(f1 int, f2 varchar(30))", otl_exception::enabled);  // create table

		//insert(); // insert records into table

		//otl_stream o_insert(1, "insert into test_tab values(10, 'AAAAA')", db);
		//char tmp[32];
// 		for(int i=1;i<=10;++i)
// 		{
// 			sprintf(tmp,"Name%d",i);
// 			o_insert<<(float)i<<tmp;
// 		}
		//select(); // select records from table


		//select LINE_ID, STR_COLUMN from TABLE_TEST_DAITEST;
		//otl_stream select_stream(50,"select * from test_tab where f1>=:f<int> and f1<=:ff<int>*2",db); 
		//otl_stream select_stream(50,"select * from test_tab",db); 
		//otl_stream select_stream(50,"select LINE_ID, STR_COLUMN from TABLE_TEST_DAITEST where LINE_ID > 995",db); 

		std::string strSQL = "select sc.NAME as HOSTNAME, mp.DEFAULT_OPERATING_MODE, et.NAME as AGNENTNAME, et.PKEY as ENTITYKEY \
from sn_system_controller sc, sn_managed_process mp, entity et \
where sc.SNSCON_ID = mp.SNSCON_ID and mp.MANAGED_PROCESS_PKEY = et.PARENTKEY and et.NAME = 'OccRightsAgent'";

		//db.auto_commit_off();
		otl_stream select_stream(50, strSQL.c_str(),db); 
		//db.commit();
		// create select stream

		char sz_HOSTNAME[512];
		double n_DEFAULT_OPERATING_MODE = 0;
		char sz_AGNENTNAME[512];
		double f_ENTITYKEY = 0;

		while(!select_stream.eof())
		{ // while not end-of-data
			memset(sz_HOSTNAME, 0, 512);
			n_DEFAULT_OPERATING_MODE = 0;
			memset(sz_AGNENTNAME, 0, 512);
			f_ENTITYKEY = 0;

			select_stream>>sz_HOSTNAME;
			select_stream>>n_DEFAULT_OPERATING_MODE;
			select_stream>>sz_AGNENTNAME;
			select_stream>>f_ENTITYKEY;
			std::cout<<"sz_HOSTNAME="<<sz_HOSTNAME<<", n_DEFAULT_OPERATING_MODE="<<n_DEFAULT_OPERATING_MODE
				<<",sz_AGNENTNAME="<<sz_AGNENTNAME<<",f_ENTITYKEY="<<f_ENTITYKEY<<std::endl;
		}
...
// 		char f2[31];
		...
		//select_stream<<8<<8; // assigning :f = 8; :ff = 8
...
//////////////////////////////////////////////////////////////////////////
int CTestWorkerOTLODBC::do_disconnect_to_db()
{
	int nFunRes = 0;
...
int CTestWorkerOTLODBC::do_connect_to_db()
{
	int nFunRes = 0;
	bool bFunRes = false;
...
int CTestWorkerOTLODBC::do_delete_table()
{
	int nFunRes = 0;
	bool bFunRes = false;
	std::string strSQL;
...
int CTestWorkerOTLODBC::do_create_table()
{
	int nFunRes = 0;
	bool bFunRes = false;
	std::string strSQL;
...
int CTestWorkerOTLODBC::do_insert_data(int nCount)
{
	int nFunRes = 0;
	bool bFunRes = false;
	std::string strSQL;
...
int CTestWorkerOTLODBC::do_select_data()
{
	int nFunRes = 0;
	bool bFunRes = false;
	std::string strSQL;
