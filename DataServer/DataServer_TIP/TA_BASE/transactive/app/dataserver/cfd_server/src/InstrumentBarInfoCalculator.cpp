...
int CInstrumentBarInfoCalculator::_InitBarCalculator()
{
	...
	m_pBarCalculator->onNewBar = boost::bind(&CInstrumentBarInfoCalculator::HandleNewBar, this, _1, _2);
	m_pBarCalculator->onBarUpdate = boost::bind(&CInstrumentBarInfoCalculator::HandleUpdateBar, this, _1, _2);
...
void CInstrumentBarInfoCalculator::HandleUpdateBar(int interval, const Bar &bar)
{
	...
	if (iterMap != m_pMapTimeBarInfo->end())
	{
		...
		//log bar
		sreaamTmp.str("");
		sreaamTmp<<"UpdateBar bar InstrumentID="<<m_nInstrumentID<<" ";
		strLogInfo = sreaamTmp.str();
		m_pUtilityFun->logBarInfo(strLogInfo, interval, pBar);
		//reset bar
		(*pBar) = bar;
...
int CInstrumentBarInfoCalculator::onMarketDataUpdateForTick(const MarketData& marketData)
{
	...
	if (enumMysqlDb == m_DBOperParam.m_nDataSrcDbType)
	{
		return nFunRes;
	}
