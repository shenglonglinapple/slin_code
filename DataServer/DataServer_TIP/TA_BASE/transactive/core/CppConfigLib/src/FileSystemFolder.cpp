#include "core/CppConfigLib/src/FileSystemFolder.h"
#include <iostream>
#include <fstream>
#include <sstream>
#include <list> 
...
#include "core/CppConfigLib/src/FileSystemItem.h"
...
CFileSystemFolder::CFileSystemFolder( void )
{
	BOOST_LOG_FUNCTION();
...
bool CFileSystemFolder::checkDirectory(const std::string& strFolderPath)
{
...
int CFileSystemFolder::createDirectory(const std::string& strFolderPath)
{
	int nFunRes = 0;
...
int CFileSystemFolder::getAllFileSystemItemInPath( const std::string& strFolderPath, MapTimeFileSystemItemT& mapTimeFileSystemItem )
{
	BOOST_LOG_FUNCTION();
	int nFunRes = 0;
...
int CFileSystemFolder::removeOldFile(time_t nStarTime, MapTimeFileSystemItemT& mapTimeFileSystemItem )
{
	BOOST_LOG_FUNCTION();
	int nFunRes = 0;
...
int CFileSystemFolder::_GetAllFileNameInPath( const std::string& strFolderPath, LstFileSystemItemsInPathT& lstFileSystemItemsInPath )
{
	BOOST_LOG_FUNCTION();
	int nFunRes = 0;
	boost::filesystem::path fFullPath = boost::filesystem::system_complete(boost::filesystem::path(strFolderPath, boost::filesystem::native ));
...
int CFileSystemFolder::_SortAllFileInPathByTime(LstFileSystemItemsInPathT& lstFileSystemItemsInPath, MapTimeFileSystemItemT& mapTimeFileSystemItem)
{
	BOOST_LOG_FUNCTION();
	int nFunRes = 0;
	LstFileSystemItemsInPathIterT   iterLst;
	CFileSystemItem* pFileSystemItem = NULL;
	std::time_t nFileNameTImeValue = 0;

	iterLst = lstFileSystemItemsInPath.begin();
	while (iterLst != lstFileSystemItemsInPath.end())
	{
		pFileSystemItem = (*iterLst);
		nFileNameTImeValue = pFileSystemItem->getLastWriteTime();
...
int CFileSystemFolder::freeData(MapTimeFileSystemItemT& mapTimeFileSystemItem )
{
	BOOST_LOG_FUNCTION();
	int nFunRes = 0;
	CFileSystemItem* pFileSystemItem = NULL;
