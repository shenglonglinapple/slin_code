...
RobotPart::RobotPart(QGraphicsItem *parent)
    : QGraphicsObject(parent), m_nColor(Qt::lightGray), m_bDragOver(false)
{
    setAcceptDrops(true);
}
//! [0]
...
void RobotPart::dropEvent(QGraphicsSceneDragDropEvent *event)
{
    m_bDragOver = false;
    if (event->mimeData()->hasColor())
        m_nColor = qvariant_cast<QColor>(event->mimeData()->colorData());
    update();
}
//! [3]
           ...
           const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    ...
    if (m_Pixmap.isNull()) 
	{
        painter->setBrush(m_bDragOver ? m_nColor.light(130) : m_nColor);
