        ...
        namespace curl_util {
            std::shared_ptr<CURL> createCurlHandle() {
                ...
                if (curlHandle.get() == nullptr) {
                    throw Exception(__FILE__, __LINE__, __FUNCTION__, "curl_easy_init() failure");
                }
                return curlHandle;
            };
            ...
            void perform(CURL *handle) {
                ...
                if (code != CURLE_OK) {
                    throw Exception(__FILE__, __LINE__, __FUNCTION__, std::string("curl_easy_perform error: ") + curl_easy_strerror(code));
                }
            }
