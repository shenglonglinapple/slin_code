...
CREATE FUNCTION  `func_is_plan_edit_locked`(v_node_id     INT(9),
                                                    v_time_out    bigint(20)) RETURNS int(2)
    DETERMINISTIC
BEGIN
# Will Aug 2011
      ...
      ELSE
         ...
         IF (SYSDATE() > (l_time_stamp + interval v_time_out second))
         THEN
            SET l_return = 0;
         END IF;
      END IF;
...
BEGIN
 ...
 the_report_loop: LOOP
  ...
  BEGIN
   ...
   VALUES(uuid(), v_NewReportKey, lv_datanode_id, lv_datapoint_id);
  END;
...
BEGIN
	DECLARE v_jobseq       INT(11) DEFAULT 0;  #sequential number of a call to this procedure
	DECLARE v_jobstart     DATETIME DEFAULT SYSDATE();  #procedure start date and time
	DECLARE v_jobtimeofday INT(11) DEFAULT UNIX_TIMESTAMP(sysdate())-UNIX_TIMESTAMP(current_date());

	DECLARE v_sourceview VARCHAR(30);  #name of source view of designated table
	DECLARE v_intervalseconds INT(11);  #interval (seconds) for recording into designated table

	DECLARE v_sourcetable VARCHAR(30);  #source table of the pi_source view

	DECLARE v_sourcedate DATETIME DEFAULT NULL;  #date of next interval (trunc(sysdate) if null) in the designated table
	DECLARE v_timeofday  INT(11);  #time in seconds (0 if null) for the next interval in the designated table
	DECLARE v_datetime   DATETIME;  #start of next (date and time) interval in the designated table

	DECLARE v_dateinsource DATETIME;  #date of last interval (trunc(sysdate) if null) in the source table of the source view of the designated table
	DECLARE v_timeinsource INT(11);  #max populated seconds (0 if null) for the last populated date in the source table of the source view of the designated table
	DECLARE v_sourcedatetime DATETIME;  #datetime of the latest entry (or entries) in the source table of the source view of the designated table
	...
	DECLARE v_diff INT(9);  #number of iterations to be performed on the designated table to get it up to date, or in line with the source table
	DECLARE v_rows INT(9) DEFAULT 0;  #number of rows processed by the insert statement

  #DECLARE v_lock  VARCHAR(2000) DEFAULT '';
  DECLARE v_lock_int INT(9);

	# Can disable the 'Handler' for debugging
	...
	out_of_here:
	BEGIN
		#get a job sequence number for this execution of the package:
		SELECT func_seq_get_next_number('DR_JOB_SEQ') INTO v_jobseq ;

		#get sourceview and interval for designated table and build first part of insert statement:
    ...
    #get the name of the source table of the source view of the designated table:
		SELECT source_name INTO v_sourcetable FROM dr_trend_config WHERE UPPER(object_name) = UPPER(v_sourceview);


    #get max populated date (min source date if null) from the designated table:
		SET @v_select = CONCAT('SELECT MAX(sourcedate) INTO @v_sourcedate FROM ' , pi_table);
		PREPARE stmt FROM @v_select;
		EXECUTE stmt;
		DEALLOCATE PREPARE stmt;
    set @v_select=null;
	set v_sourcedate = @v_sourcedate;
	set @v_sourcedate=null;

		IF v_sourcedate IS NULL or v_sourcedate = '0000-00-00 00:00:00' THEN
      ...
      IF v_sourcedate IS NULL or v_sourcedate = '0000-00-00 00:00:00' THEN  #exit if the source is empty
        leave out_of_here;
      END IF;
		END IF;

    #deterimine the next interval to be calculated and inserted into the designated table:
		SET @v_select = CONCAT('SELECT MAX(timeofday) +  ',v_intervalseconds,' INTO @v_timeofday FROM ', pi_table, ' WHERE sourcedate = ''',v_sourcedate,'''');
		PREPARE stmt FROM @v_select;
		EXECUTE stmt;
		DEALLOCATE PREPARE stmt;
    set @v_select = null;
	set v_timeofday = @v_timeofday;
	set @v_timeofday = null;

		IF v_timeofday IS NULL THEN # '' of int = 0
			SET v_timeofday = v_intervalseconds;
		END IF;

    #adjust next interval if rolled over into the new date:
		IF v_timeofday >= 86400 THEN
			SET v_sourcedate = adddate(v_sourcedate, interval 1 day);
			SET v_timeofday  = 0;
		END IF;

    #calculate timestamp of the next interval:
		SET v_datetime = adddate(v_sourcedate, interval v_timeofday second);
    ...
    set @v_select = null;
	set v_dateinsource = @v_dateinsource;
	set @v_dateinsource = null;

		IF v_dateinsource IS NULL or v_dateinsource = '0000-00-00 00:00:00' THEN
			LEAVE out_of_here;
		END IF;

    #get max populated seconds for the last populated date from the source table of the source view of the designated table:
		SET @v_select = CONCAT('SELECT MAX(timeofday) INTO @v_timeinsource FROM ', v_sourcetable, ' WHERE sourcedate = ''',v_dateinsource,'''');
		PREPARE stmt FROM @v_select;
		EXECUTE stmt;
		DEALLOCATE PREPARE stmt;
    set @v_select = null;
	set v_timeinsource = @v_timeinsource;
	set @v_timeinsource = null;

		
		IF v_timeinsource IS NULL THEN
			SET v_timeinsource = v_jobtimeofday;
		END IF;


    #calculate actual timestamp of the last populated interval in the source table of the source view of the designated table:
		SET v_sourcedatetime = adddate(v_dateinsource, interval v_timeinsource second);


    #determine number of intervals to be populated:
		SET v_diff = FLOOR((UNIX_TIMESTAMP(v_sourcedatetime) - UNIX_TIMESTAMP(v_datetime)) / v_intervalseconds);
		IF v_diff > 0  THEN
			WHILE v_diff > 0 DO
            start transaction;
                #LOCK TABLE DR_EVENT IN ROW SHARE MODE;
                select count(*) into v_lock_int from DR_TREND_INTERVAL lock in share mode;

                 SET @v_lock = concat('select count(*) into @v_lock_int from ',v_sourcetable,' lock in share mode');
                 PREPARE stmt from @v_lock;
                 execute stmt;
                 deallocate prepare stmt;
                 set @v_lock= null;
                 

                 SET @v_lock = concat('select count(*) into @v_lock_int from ',pi_table,' lock in share mode');
                 PREPARE stmt from @v_lock;
                 execute stmt;
                 deallocate prepare stmt;
                 set @v_lock=null;
...
# Modified Sept 2011
  ...
  SET l_return = 0;
    ...
    WHERE p_key = v_node_id;
   		...
   		IF (SYSDATE() > (l_time_stamp + interval v_time_out second)) THEN
   	  	  	SET l_return = 1;
		END IF;
	END IF;
