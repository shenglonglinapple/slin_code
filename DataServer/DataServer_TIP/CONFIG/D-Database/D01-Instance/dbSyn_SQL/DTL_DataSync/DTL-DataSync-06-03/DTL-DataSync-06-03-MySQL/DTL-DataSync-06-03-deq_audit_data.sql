...
--	DECLARE  v_deq_msg             VARCHAR(8080) DEFAULT NULL;
	...
	SELECT MSGID, MSG_SID, USER_DATA_1, USER_DATA_3 FROM audit_data_queue_t WHERE state = 1 and sub_cnt = 0 and msg_sid between v_min_sid and v_max_sid ORDER BY msg_sid ASC;
    ...
    BEGIN
		...
		INSERT INTO error_log(pkey,sql_statement,error_message,timestamp)
			VALUES(func_seq_get_next_number('ERRLOG_SEQ'), concat(v_queue,'@msg_sid',ifnull(v_msg_sid,''),'of',ifnull(v_min_sid,''),'~',ifnull(v_max_sid,''),':',
				v_message_qualifier,'==>',substr(ifnull(v_sql,''),1,3500)),concat('ERROR 1213 (40001): Deadlock found of prc_deq_audit_data_queue at: ', DATABASE() ),SYSDATE());
...
--		FETCH cu_deq_data INTO v_msgid, v_msg_sid, v_message_qualifier, v_deq_msg;				
		FETCH cu_deq_data INTO v_msgid, v_msg_sid, v_message_qualifier, v_sql;				
...
*/	
	...
	IF v_message_qualifier = gc_halt_dequeue_message THEN
