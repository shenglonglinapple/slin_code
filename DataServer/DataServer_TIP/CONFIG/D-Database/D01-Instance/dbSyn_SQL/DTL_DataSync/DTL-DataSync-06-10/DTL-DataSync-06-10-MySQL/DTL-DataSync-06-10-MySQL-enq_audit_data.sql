...
#	DECLARE    v_mess_qualifier_len  INT(5) DEFAULT 0;
  ...
  -- Add by will 19Oct2012 for handle deadlock, the whole transaction will be rollback.
  -- For other EXCEPTION, just raise error without rollback.
  ...
  -- Add by Will 20May2013 to record the error for data missing troubleshooting. (Apply successful in local but insert into queue failed)
  ...
  DECLARE CONTINUE HANDLER FOR SQLEXCEPTION
	...
	insert into error_log(pkey,sql_statement,error_message,timestamp)
		values (func_seq_get_next_number('errlog_seq'),concat('Enqueue:',p_message_qualifier,'@DB name:',database(),'@p_location:',p_location,'@v_recipient:',v_recipient),
                concat('Got error when call prc_enqueue_audit_data::p_audit_data_MySQL:',p_audit_data_MySQL),now());
	commit;
	resignal;
    end;
...
*/
	SET	 v_message_qualifier = CONCAT(p_message_qualifier,':',p_location,',',@@server_id,',0');
	# CALL prc_enqueue(v_queue_name, v_message_qualifier, p_audit_data_Oracle, p_audit_data_MySQL, null, 1, v_recipient);
	CALL prc_enqueue_notrans(v_queue_name, v_message_qualifier, p_audit_data_Oracle, p_audit_data_MySQL, null, 1, v_recipient);
