...
BEGIN
	...
	SELECT COUNT(*) AS CNT FROM adm_sequence_rep;
	
	DECLARE c_seqadm_maxpkey CURSOR FOR
	SELECT MAX(PKEY) FROM adm_sequence_rep;
...
START TRANSACTION;
	SET @g_sql = CONCAT('CREATE TABLE ', v_queue_table, ' (
				...
				`user_data_1` varchar(4000) DEFAULT NULL,
				`user_data_2` varchar(4000) DEFAULT NULL,
				`user_data_3` varchar(4000) DEFAULT NULL,
				PRIMARY KEY (`msgid`),
