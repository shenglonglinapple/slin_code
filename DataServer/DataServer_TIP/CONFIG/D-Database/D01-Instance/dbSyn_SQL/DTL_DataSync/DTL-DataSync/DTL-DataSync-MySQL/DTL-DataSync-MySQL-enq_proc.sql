...
insert into adm_sequence values (123+1,'AQ_LID_SEQ',1,0,4294967290,1,'Y');
insert into adm_sequence values (123+2,'AQ_HID_SEQ',1,0,4294967290,1,'Y');
insert into adm_sequence values (123+3,'AQ_ENQ_LID_SEQ',1,0,4294967290,1,'Y');
insert into adm_sequence values (123+4,'AQ_ENQ_HID_SEQ',1,0,4294967290,1,'Y');
...
BEGIN
    ...
    SELECT PKEY FROM recipient_all WHERE UPPER(NAME) = UPPER(p_subname);
...
#	SET @g_maxtid   = NULL;
  ...
  set @g_sql=null;
	...
	END IF;
  */
	SET @g_sql = CONCAT('INSERT INTO ', v_enq_table, '(SUB_ID,SUB_NAME,ENQ_HID,ENQ_LID,MSGID) VALUES (', v_sub_id,',', '''',v_subname,'''', ',',v_hid,',',v_lid,',''',p_msgid,'''',')');
...
*/
	...
	IF(p_subtype = v_subtypelocal) THEN
	-- the message received from remote and the receiver is the local.
	-- enq one message only
		SET v_local_db = DATABASE();
		CALL prc_enq_table(p_queue_table, p_msgid, v_local_db);
	ELSE
	...
	-- check the msg_identifier, should only be (INSERT OR GROUP)
      ...
      ELSEIF upper(p_msg_identifier) = 'PUBLIC' THEN
			  ...
			  the_public_mem_loop: LOOP
			  	...
			  	END IF;
  			CALL prc_enq_table(p_queue_table, p_msgid, v_public_mem);
      ...
      ELSEIF upper(p_msg_identifier) in ('INSERT', 'UPDATE', 'DELETE') THEN
          ...
          END IF;
			  ...
			  the_other_mem_loop: LOOP
			  	...
			  	END IF;
  			CALL prc_enq_table(p_queue_table, p_msgid, v_other_mem);
...
BEGIN
  ...
  if  upper(p_message_qualifier) = 'PUBLIC' then
    select name,count(*) into v_sub_mem,v_pub_cnt from recipient where address is not null and status = 0 group by name;
      if v_sub_mem is not null and (v_pub_cnt <> 1 or upper(v_sub_mem)<>v_centre_db) THEN
         set v_sub_cnt = 2;
      end if;
  elseif upper(p_message_qualifier) = 'GROUP' then
    select member,count(*) into v_sub_mem,v_grp_cnt from recipient_group where UPPER(NAME) = UPPER(p_location) AND UPPER(member) <> UPPER(database()) and status = 0  group by member;
      if v_sub_mem is not null and (v_grp_cnt <> 1 or upper(v_sub_mem)<>v_centre_db) THEN
         set v_sub_cnt = 2;
      end if;
  elseif upper(p_message_qualifier) in ('INSERT', 'UPDATE', 'DELETE') then
    select name,count(*) into v_sub_mem,v_oth_cnt from recipient WHERE address is not null and status = 0 group by name;
      if v_sub_mem is not null and (v_oth_cnt <> 1 or upper(v_sub_mem)<>v_centre_db) THEN
         set v_sub_cnt = 2;
      end if;
  ...
  IF p_audit_data_MySQL IS NOT NULL and p_audit_data_MySQL <> ''  THEN
  ...
  IF v_qua_chk = 1 and v_pub_cnt <> 0 and v_grp_cnt <> 0 and v_oth_cnt<>0 and v_sub_mem is not null then
...
BEGIN
	...
	DECLARE  v_state        	TINYINT(1) DEFAULT 1;         -- 1: ready; 2: processing; 3: done
	DECLARE  v_maxInt       	INT(10) unsigned DEFAULT 4294967290;
    ...
    Begin
      ROLLBACK;
      INSERT INTO error_log(pkey,sql_statement,error_message,timestamp)
      VALUES(func_seq_get_next_number('ERRLOG_SEQ'), concat(p_msg_identifier,'@',substr(p_payload,1,3500)),concat('WARNING! SQLEXCEPTION of prc_enqueue at: ',DATABASE(), ', and para is: ',p_location),SYSDATE());
    end;
...
start transaction;
    set v_lid = func_seq_get_next_number('AQ_LID_SEQ');
		...
		SET v_queue_table = UPPER(p_queue_table);
		SET v_queue_name = LEFT(v_queue_table, CHAR_LENGTH(v_queue_table) - 2);		
		SET v_msgid      = UUID();
		SET v_msg_data   = REPLACE(p_payload, '''', '''''');
    if p_msgid = '' or p_msgid is null then
        SET @g_sql = CONCAT('INSERT INTO ', p_queue_table, '(Q_NAME, ENQ_HID, ENQ_LID, MSGID, PRIORITY, STATE, SUB_CNT, ENQ_TIME, ENQ_UID, SENDER_MSGID, USER_DATA) VALUES (',
							'''', v_queue_name, '''', ',', v_hid, ',', v_lid, ',','''',v_msgid,'''' ,',', v_priority, ',', v_state,',', v_sub_cnt, ', sysdate(),SUBSTRING_INDEX(CURRENT_USER, ', '''',v_curuser_deli,'''', ',1),',
							'NULL',',' ,'''', v_msg_data, '''',')');
    else
		    SET @g_sql = CONCAT('INSERT INTO ', p_queue_table, '(Q_NAME, ENQ_HID, ENQ_LID, MSGID, PRIORITY, STATE, SUB_CNT, ENQ_TIME, ENQ_UID, SENDER_MSGID, USER_DATA) VALUES (',
							'''', v_queue_name, '''', ',', v_hid, ',', v_lid, ',','''',v_msgid,'''' ,',', v_priority, ',', v_state,',', v_sub_cnt, ', sysdate(),SUBSTRING_INDEX(CURRENT_USER, ', '''',v_curuser_deli,'''', ',1),',
    ...
    set @g_sql = null;
		...
		CALL prc_enq_others(p_location, p_queue_table, p_msg_identifier, v_msgid, p_subtype, p_center_db);
#	END IF;
