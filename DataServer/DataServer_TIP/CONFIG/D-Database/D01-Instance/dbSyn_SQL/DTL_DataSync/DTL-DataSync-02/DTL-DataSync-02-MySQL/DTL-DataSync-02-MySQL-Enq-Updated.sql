...
*/
  ...
  set @g_sql=null;
	...
	END IF;
  */
	SET @g_sql = CONCAT('INSERT INTO ', v_enq_table, '(SUB_ID,SUB_NAME,ENQ_HLID,MSGID,STATE) VALUES (', v_sub_id,',', '''',v_subname,'''', ',',p_hlid,',''',p_msgid,'''',',1)');
...
*/
	...
	-- enq one message only
		...
		CALL prc_enq_table(p_queue_table, p_msgid, v_local_db, p_hlid);
	ELSE
	...
	-- check the msg_identifier, should only be (INSERT OR GROUP)
      ...
      IF upper(p_msg_identifier) = 'GROUP' THEN
			  ...
			  the_group_mem_loop: LOOP
			  	...
			  	END IF;
			  	CALL prc_enq_table(p_queue_table, p_msgid, v_group_mem, p_hlid);
      ...
      ELSEIF upper(p_msg_identifier) = 'PUBLIC' THEN
			  ...
			  the_public_mem_loop: LOOP
			  	...
			  	END IF;
  			CALL prc_enq_table(p_queue_table, p_msgid, v_public_mem, p_hlid);
      ...
      ELSEIF upper(p_msg_identifier) in ('INSERT', 'UPDATE', 'DELETE') THEN
          ...
          END IF;
			  ...
			  the_other_mem_loop: LOOP
			  	...
			  	END IF;
  			CALL prc_enq_table(p_queue_table, p_msgid, v_other_mem, p_hlid);
...
start transaction;
    ...
    SET v_hlid = concat(v_hid,v_lid);
		CALL prc_enq_others(p_location, p_queue_table, p_msg_identifier, v_msgid, p_sub_cnt, p_center_db, v_hlid);
...
BEGIN
 ...
 ;
  ...
  CALL prc_enqueue(p_location, v_queue_table, p_message_qualifier, v_payload, '', v_centre_db, v_sub_cnt);
